...
previsores15 = [] #armazenará 90 primeiros dados (tempo = 90)
preco_real15 = []

for i in range(15, 1242):
    previsores15.append(base_treinamento_norm[i-15:i, 0])
    preco_real15.append(base_treinamento_norm[i, 0])

previsores90, preco_real = np.array(previsores), np.array(preco_real)
previsores = np.reshape(previsores, (previsores.shape[0], previsores.shape[1], 1))

regressor = Sequential()
regressor.add(LSTM(units = 100, return_sequences = True, input_shape = (previsores.shape[1], 1))) #units = numero de cel de memorias
...
regressor.fit(previsores, preco_real, epochs = 100, batch_size = 32) #minimo de 100 epocas
